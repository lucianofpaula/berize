...
export function ServicoForm({ initialData }: { initialData: InitialData }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!name || !price || parseFloat(price) <= 0 || !duration || parseInt(duration) <= 0) return

    setEnviando(true)

    const res = await fetch(`/api/servicos/${initialData.id}`, {
      method: "PUT",
    ...
    if (!res.ok) {
      ...
      setErro(data.error ?? "Erro ao salvar serviço.")
      setEnviando(false)
      return
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 shadow-sm space-y-5">
        <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Dados do serviço</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <div className="relative">
              <ClockIcon className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-zinc-400" />
              <input
                type="number"
                min="1"
                value={duration}
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          type="submit"
          disabled={enviando || !name || !price || parseFloat(price) <= 0 || !duration || parseInt(duration) <= 0}
